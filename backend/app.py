"""
Collection of different APIs which access the MySQL database in the backend
The APIs below are grouped by type.
To create a new API, just use these templates.
Database calls are done using functions defined in the db.py module

POST(like)
@app.route('/my/endpoint',methods=['POST'])
def some_random_name():
    received_body = request.get_json()
    value = received_body['value_in_json']
    ...
    result_dictionary = ...
    response_valid_request(result_dictionary)

GET(like)
@app.route('/my/endpoint',methods=['GET'])
def some_random_name():
    value = request.args.get('value_in_url')
    ...
    result_dictionary = ...
    response_valid_request(result_dictionary)
"""
...
#Function returning "Unauthorized request" from this server
...
#Function returning "Valid Request" from this server
...
# Get or create a new offer for a vendor
